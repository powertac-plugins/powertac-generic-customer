...
class GenericConsumerTests extends GroovyTestCase {
  ...
  protected void setUp()
  {
    ...
    if (Competition.count() == 0) {
      comp = new Competition(name: 'generic-consumer-test')
      assert comp.save()
    }
    else {
  ...
  void testCreationAndSubscriptionToDefault()
  {
    ...
    consumer = new GenericConsumer(customerInfo: customerInfo)
    consumer.init()
    consumer.subscribeDefault()
    if (!consumer.validate()) {
      consumer.errors.each { println it.toString() }
      fail("Could not save consumer")
    }
  ...
  void testPowerConsumption()
  {
    ...
    consumer.consumePower()

    assertFalse("Customer consumed power", consumer.subscriptions?.totalUsage == 0)
  ...
  void testRevokingSubscriptions()
  {
    ...
    consumer.unsubscribe(tsd,70)
    consumer.subscribe(tc1, 23)
    consumer.subscribe(tc2, 23)
    consumer.subscribe(tc3, 24)
    TariffSubscription ts1 =
        TariffSubscription.findByTariffAndCustomer(tc1, consumer)
    consumer.unsubscribe(ts1, 12)
    TariffSubscription ts2 =
        TariffSubscription.findByTariffAndCustomer(tc2, consumer)
    consumer.unsubscribe(ts2, 11)
    TariffSubscription ts3 =
        TariffSubscription.findByTariffAndCustomer(tc3, consumer)
    consumer.unsubscribe(ts3, 20)

    println(TariffSubscription.count())

    assertEquals("4 Subscriptions for consumer",3, consumer.subscriptions?.size())
    ...
    consumer.checkRevokedSubscriptions()
