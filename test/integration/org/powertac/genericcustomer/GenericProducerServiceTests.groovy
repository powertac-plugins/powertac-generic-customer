...
class GenericProducerServiceTests extends GroovyTestCase {

  def timeService
  def tariffMarketService
  def tariffMarketInitializationService
  def genericProducerService
  def genericProducerInitializationService
  ...
  protected void setUp() {
    super.setUp()
    PluginConfig.findByRoleName('GenericProducer')?.delete()

    // create a Competition, needed for initialization
    if (Competition.count() == 0) {
      comp = new Competition(name: 'generic-producer-service-test')
      assert comp.save()
    }
    else {
  ...
  void initializeService () {
    ...
    PluginConfig config = PluginConfig.findByRoleName('GenericProducer')
    config.configuration['numberOfProducers'] = '2'
    genericProducerInitializationService.initialize(comp, [
      'TariffMarket',
      'DefaultBroker'
    ])
  }
  ...
  void testNormalInitialization () {
    ...
    def result = genericProducerInitializationService.initialize(comp, [
      'TariffMarket',
      'DefaultBroker'
    ])
    assertEquals("correct return value", 'GenericProducer', result)
    //assertEquals("correct number of customers", 0, GenericProducer.count())
  }
  ...
  void testServiceInitialization() {
    initializeService()
    assertEquals("Two Customers Created", 2, GenericProducer.count())
    // customers no longer subscribe at init()
    tariffMarketService.publishTariffs()
    assertFalse("Producer 1 subscribed", GenericProducer.findByCustomerInfo(CustomerInfo.findByName("GenericProducer 1")).subscriptions == null)
    assertFalse("Producer 2 subscribed", GenericProducer.findByCustomerInfo(CustomerInfo.findByName("GenericProducer 2")).subscriptions == null)
    assertFalse("Producer 1 subscribed to default", GenericProducer.findByCustomerInfo(CustomerInfo.findByName("GenericProducer 1")).subscriptions == tariffMarketService.getDefaultTariff(PowerType.PRODUCTION))
    assertFalse("Producer 2 subscribed to default", GenericProducer.findByCustomerInfo(CustomerInfo.findByName("GenericProducer 2")).subscriptions == tariffMarketService.getDefaultTariff(PowerType.PRODUCTION))
  }
  ...
  void testPowerConsumption() {
    ...
    genericProducerService.activate(timeService.currentTime, 1)
    ...
    assertEquals("Tariff Transactions Created", GenericProducer.count(), TariffTransaction.findByTxType(TariffTransactionType.PRODUCE).count())
  }
  ...
  void testTariffPublication() {
    ...
    // it's 13:00
    genericProducerService.activate(timeService.currentTime, 1)
    genericProducerService.activate(timeService.currentTime, 2)
